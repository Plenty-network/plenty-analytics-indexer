...
function build({ dbClient }: Dependencies): Router {
  ...
  router.get("/", async (_: Request, res: Response) => {
    try {
      ...
      // Fetch tvl value at an hour
      async function getTvlValueHour(ts: number) {
        return await dbClient.get({
          table: "plenty_aggregate_hour",
          select: "tvl_value as tvl",
          where: `ts=(SELECT MAX(ts) FROM plenty_aggregate_hour WHERE ts<=${ts})`,
        });
      }
      ...
      const _entry = await dbClient.get({
        ...
        select: `
            ...
            tvl_value
          `,
        where: `ts>=${t365} AND ts<=${t0} ORDER BY ts`,
      });
      aggregate1Y = _entry.rows;

      // Get tvl value at current hour and 24 hours ago
      const tvl24H = parseFloat((await getTvlValueHour(t24h)).rows[0].tvl ?? 0);
      const tvlCH = parseFloat((await getTvlValueHour(tch)).rows[0].tvl ?? 0);
      ...
      const plenty: PlentyResponse = {
        ...
        tvl: {
          ...
          change24H: percentageChange(tvl24H, tvlCH),
          history: tvlHistory,
        },
      };
